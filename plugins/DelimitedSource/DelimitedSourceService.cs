...
public sealed class DelimitedSourceService : IPluginService
{
    ...
    public async Task<IDataset> ReadFileAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Reading delimited file: {FilePath}", config.FilePath);
        ...
        try
        {
            ...
            // Simple CSV reading with CsvHelper
            var rows = await ReadCsvRowsAsync(config, schema, cancellationToken);
