...
public sealed class DelimitedSourceService : IPluginService
{
    ...
    public Task<IChunk> ProcessChunkAsync(IChunk input, CancellationToken cancellationToken = default)
    {
    ...
    public Task<IArrayRow> ProcessRowAsync(IArrayRow input, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IDataset> ReadFileAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Validate schema is provided (schema inference not yet implemented)
            _logger.LogDebug("ReadFileAsync: Validating output schema");
            var schema = config.OutputSchema ?? throw new InvalidOperationException("Either InferSchema must be true or OutputSchema must be provided");
