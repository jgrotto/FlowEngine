...
public sealed class ErrorRecoveryService : IDisposable
{
    ...
    /// </summary>
    public async Task<RecoveryResult> AttemptSchemaRecoveryAsync(FlowEngineSchemaException exception, ErrorContext context)
    {
        try
        {
            ...
            if (exception.SourceSchema != null && exception.TargetSchema != null)
            {
                ...
                if (compatibilityResult.CanAutoResolve)
                {
                    ...
                    return RecoveryResult.Success($"Schema auto-resolved: {compatibilityResult.Resolution}");
                }
            }
            ...
            return RecoveryResult.RequiresManualIntervention(guidance);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error during schema recovery attempt");
            return RecoveryResult.Failed("Schema recovery attempt failed");
        }
    }
    ...
    /// </summary>
    public async Task<RecoveryResult> AttemptIORecoveryAsync(IOException exception, ErrorContext context)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(filePath))
            {
                ...
                if (File.Exists(filePath))
                {
                    try
                    {
                        ...
                        return RecoveryResult.Success("File access recovered");
                    }
                    catch (UnauthorizedAccessException)
                    {
                        return RecoveryResult.RequiresManualIntervention($"File access denied: {filePath}. Check permissions.");
                    }
                    catch (IOException)
                    {
                        // File might be locked, suggest retry
                        return RecoveryResult.RequiresManualIntervention($"File may be locked: {filePath}. Retry after a delay.");
                    }
                }
                else
                {
                    return RecoveryResult.RequiresManualIntervention($"File not found: {filePath}. Verify path and file existence.");
                }
            }
            ...
            {
                return RecoveryResult.RequiresManualIntervention("File sharing violation detected. Retry after a delay or ensure file is not in use.");
            }
            
            return RecoveryResult.RequiresManualIntervention($"I/O error requires manual investigation: {exception.Message}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error during I/O recovery attempt");
            return RecoveryResult.Failed($"I/O recovery attempt failed: {ex.Message}");
        }
    }
    ...
    private async Task<RecoveryResult> AttemptPluginLoadRecoveryAsync(FlowEnginePluginException exception)
    {
        // Check if the assembly file exists
        if (!string.IsNullOrEmpty(exception.AssemblyPath) && !File.Exists(exception.AssemblyPath))
        {
            return RecoveryResult.RequiresManualIntervention($"Plugin assembly not found: {exception.AssemblyPath}");
        }
        
        // Check for common plugin load issues
        if (exception.InnerException is FileLoadException)
        {
            return RecoveryResult.RequiresManualIntervention("Plugin assembly cannot be loaded. Check dependencies and .NET version compatibility.");
        }
        
        if (exception.InnerException is BadImageFormatException)
        {
            return RecoveryResult.RequiresManualIntervention("Plugin assembly is corrupted or incompatible. Rebuild or reinstall the plugin.");
        }
        
        return RecoveryResult.RequiresManualIntervention("Plugin load failure requires manual investigation of assembly and dependencies.");
    }

    private async Task<RecoveryResult> AttemptPluginExecutionRecoveryAsync(FlowEnginePluginException exception)
    {
        // Plugin execution failures might be transient
        return RecoveryResult.RequiresManualIntervention($"Plugin execution failed: {exception.Message}. Review plugin logs and input data.");
    }

    private async Task<RecoveryResult> AttemptPluginConfigurationRecoveryAsync(FlowEnginePluginException exception)
    {
        return RecoveryResult.RequiresManualIntervention($"Plugin configuration is invalid: {exception.Message}. Review and correct plugin configuration.");
    }
